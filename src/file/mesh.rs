...
impl LodGroup {
    pub fn read<F: Read + Seek>(file: &mut F) -> Result<Self> {
        ...
        let mesh_group_offsets = (0..count).map(|_| { file.read_u64() }).collect::<io::Result<Vec<u64>>>()?;
...
impl Mesh {
    pub fn new<F: Read + Seek>(mut file: F) -> Result<Self> {
        ...
        let lod_group_offsets = (0..lod_group_count).map(|_| { file.read_u64() }).collect::<io::Result<Vec<u64>>>()?;
        ...
        let name_offsets = (0..name_count).map(|_| { file.read_u64() }).collect::<io::Result<Vec<_>>>()?;
