...
impl<const N: usize> AsRef<[u8; N]> for Magic<N> {
    fn as_ref(&self) -> &[u8; N] {
        &self.0
    }
}
...
impl FileReader {
    ...
    pub fn dump_file(&mut self, file: std::path::PathBuf) -> Result<()> {
        ...
        match file_ext {
            ...
            "pog" => {
                Pog::dump(&file_path, &output_path)?;
            }
            "bin" => {
                if let Some(steamid) = &self.steamid {
                    ...
                    let mut _save = SaveFile::read(&mut reader, &mut SaveContext{key: Some(steamid), game: Game::MHWILDS, curve_index: None }, )?;
                    /*println!("here\n");
                    ...
                    let _ = SaveFile::read(&mut reader, &mut SaveContext{key: None, game: Game::MHWILDS, curve_index: None});
