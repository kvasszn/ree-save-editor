...
use super::util::{scalar_mult, point_add};
use hex_literal::hex;
...
impl Citrus {
    ...
    const BLOCK_SIZE: usize = Self::KEY_IV_SIZE + Self::ENC_KEYS_SIZE + Self::ENC_DATA_SIZE + Self::HASH_SIZE;
    ...
    fn decrypt_ec_elgamal(&self, c1: (Integer, Integer), c2: (Integer, Integer), params: &CurveParams) -> Option<(Integer, Integer)> {
    ...
    fn ecc_decrypt_keys(&self, buf: &[u8; 512], params: &CurveParams) -> Option<([u8; 16], [u8; 16])> {
        let mut key = [0u8; 16];
        let mut iv = [0u8; 16];
        key[0..8].copy_from_slice(&self.decrypt_key_segment(&buf[0..128], params)?.to_u64_le_bytes());
        key[8..16].copy_from_slice(&self.decrypt_key_segment(&buf[128..256], params)?.to_u64_le_bytes());
    ...
    pub fn decrypt(&self, buf: &[u8], decrypted_len: usize) -> Option<Vec<u8>> {
        ...
        let mut ecc_keys = [0u8; Self::ENC_KEYS_SIZE];
        let mut dec_buf = [0u8; Self::ENC_DATA_SIZE];
        let curve_params = self.param_index
